...
import { Download, FlipHorizontal2, FlipVertical2, ImagePlus, RotateCcw, RotateCw, Scan, Trash2 } from 'lucide-react';
...
export function ImageStudio({ mode }: { mode: ImageStudioMode }) {
  ...
  async function loadFile(selected: File) {
    ...
    nextImage.onload = () => {
      ...
      setZoom(1);
      setCrop(fullCrop);
      setTargetWidth(nextImage.naturalWidth);
      setTargetHeight(nextImage.naturalHeight);
      setOutputUrl('');
      setOutputSize(0);
      setError('');
  ...
  function changeWidth(value: number) {
    ...
    if (lockAspect) setTargetHeight(Math.max(1, Math.round(width * ((rotated.height * crop.height) / (rotated.width * crop.width)))));
  }

  function changeHeight(value: number) {
    const height = Math.max(1, Math.round(value || 1));
    setTargetHeight(height);
    if (lockAspect) setTargetWidth(Math.max(1, Math.round(height * ((rotated.width * crop.width) / (rotated.height * crop.height)))));
  }
  ...
  async function exportImage() {
    ...
    try {
      ...
      sourceCanvas.width = rotated.width;
      sourceCanvas.height = rotated.height;
      drawTransformed(sourceCanvas, image, rotation, flipX, flipY, zoom, 1);
      ...
      outputCanvas.width = Math.max(1, targetWidth || sw);
      outputCanvas.height = Math.max(1, targetHeight || sh);
    ...
    return (
      <div>
        ...
        <div className="image-start-actions">
          <span>想先熟悉操作？</span>
          <button className="secondary-button" type="button" onClick={() => void createDemoImage().then(loadFile)}>使用示例图片</button>
        </div>
    ...
    <div className="image-studio">
      <div className="image-stage-card">
        <div className="image-stage-toolbar">
          ...
          <div><button type="button" onClick={() => rotate(-90)} title="向左旋转"><RotateCcw /></button><button type="button" onClick={() => rotate(90)} title="向右旋转"><RotateCw /></button><button type="button" onClick={() => setFlipX((value) => !value)} className={flipX ? 'is-active' : ''} title="水平翻转"><FlipHorizontal2 /></button><button type="button" onClick={() => setFlipY((value) => !value)} className={flipY ? 'is-active' : ''} title="垂直翻转"><FlipVertical2 /></button><button type="button" onClick={reset} title="移除图片"><Trash2 /></button></div>
        </div>
        <div className="image-stage-viewport">
          <div ref={stageRef} className="image-stage">
            ...
            <div className="crop-box" style={{ left: `${crop.x * 100}%`, top: `${crop.y * 100}%`, width: `${crop.width * 100}%`, height: `${crop.height * 100}%` }} onPointerDown={(event) => startCropDrag(event, 'move')}>
              ...
              <button type="button" className="crop-handle" onPointerDown={(event) => startCropDrag(event, 'resize')} aria-label="调整裁剪区域" />
            </div>
          </div>
        </div>
        <div className="zoom-control"><Scan size={17} /><span>缩放</span><input type="range" min="1" max="3" step="0.01" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /><b>{Math.round(zoom * 100)}%</b></div>
      </div>

      <aside className="image-controls">
        <section><h3>输出尺寸</h3><div className="dimension-row"><label>宽度<input type="number" min="1" max="12000" value={targetWidth} onChange={(event) => changeWidth(Number(event.target.value))} /></label><button type="button" className={lockAspect ? 'is-active' : ''} onClick={() => setLockAspect((value) => !value)} title="锁定比例">{lockAspect ? '🔗' : '—'}</button><label>高度<input type="number" min="1" max="12000" value={targetHeight} onChange={(event) => changeHeight(Number(event.target.value))} /></label></div><button className="text-button" type="button" onClick={() => { setCrop(fullCrop); setTargetWidth(rotated.width); setTargetHeight(rotated.height); }}>恢复完整画面</button></section>
        ...
        <button className="primary-button export-button" type="button" onClick={() => void exportImage()} disabled={busy}><Download size={18} /> {busy ? '正在导出…' : '生成预览'}</button>
        {outputUrl && <section className="output-preview"><img src={outputUrl} alt="处理后的图片预览" /><div><span>{targetWidth} × {targetHeight}</span><b>{formatFileSize(outputSize)}</b></div><a className="primary-button" href={outputUrl} download={`edited.${format.split('/')[1] === 'jpeg' ? 'jpg' : format.split('/')[1]}`}><Download size={17} /> 下载图片</a></section>}
        {error && <div className="error-banner">{error}</div>}
      </aside>
    </div>
  );
...
function drawTransformed(canvas: HTMLCanvasElement, image: HTMLImageElement, rotation: number, flipX: boolean, flipY: boolean, zoom: number, scale: number) {
